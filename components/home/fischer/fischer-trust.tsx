    ...
    <section className="border-y border-fischer-line bg-paper text-anthracite">
      <div className="mx-auto max-w-360 px-6 sm:px-12 lg:px-20">
        <dl className="grid grid-cols-1 divide-y divide-fischer-line sm:grid-cols-2 sm:divide-x sm:divide-y-0 lg:grid-cols-4">
          {FACTS.map((fact) => (
            <div key={fact.value} className="py-8 sm:px-7 sm:first:pl-0 lg:py-9">
              <dt className="text-[10px] font-semibold uppercase tracking-[0.26em] text-anthracite/55">
                {fact.value}
              </dt>
              <dd className="mt-3 max-w-56 text-sm leading-relaxed text-fischer-mute">
                {fact.label}
              </dd>
            </div>
          ))}
